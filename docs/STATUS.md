...
> 최종 갱신: 2026-07-02 (Phase 8 완료 — 소비자 수집 UX·삭제·중복판정·아이콘·문서 정합화)
...
### Phase 7 — 서버 다운로드(yt-dlp/cookies) ⛔ 미채택 (superseded)
- 구현했던 `/api/download`(Node 런타임 yt-dlp) + cookies.txt 흐름은 일반 사용자 UX·운영비(0원)에
  부적합 → **미채택**. 코드는 `reference/phase7-ytdlp/` 로 분리(커밋 제외).
- 라이브러리 로드 시 작업 폴더 자동 재연결, 상세 영상 자동재생, 공통 BackButton 등
  **저장·UI 자산은 Phase 8로 승계**하여 유지.

### Phase 8 — Browser Connector 수집 + 자동 등록 ✅ (2026-07-02)
- [x] ClipMiner 확장(MV3, `poc/douyin-extractor/`): content.js 온페이지 추출 + background 오케스트레이션 + web-bridge 중계
- [x] 수집 흐름: `/download` 링크 → 확장이 Douyin 내장 데이터에서 영상 추출(content-type 검증) → cross-origin fetch → mp4 payload 회신
- [x] 저장 주체 일원화: `/download` 가 작업 폴더(File System Access)에 직접 저장 + IndexedDB 자동 등록 (`src/lib/connector.ts`)
- [x] 소비자 원클릭 UX: 링크 붙여넣기 → [영상 저장하기] (확장/추출 용어 비노출), 진행 상태 표시, 타임아웃/requestId
- [x] 확장 설치 가이드 + 배포 ZIP(`public/clipminer-extension.zip`, `poc/make-extension-zip.ps1`), 지원 브라우저/주의사항, Douyin 로그인 안내
- [x] 브라우저 다운로드 폴더 미사용(`chrome.downloads` 제거) — 작업 폴더에만 저장
- [x] 상세 UX: 미리보기 컨트롤 정리(전체화면/점점점 제거), "다운로드 관리" 카드(다운로드 완료일/표시·원본 파일명/저장 위치 안내), 추천 태그 한/중, 로컬 영상 안내 문구 정리
- [x] 삭제 시 로컬 파일 동반 삭제(확인창, best-effort, 작업 폴더 밖 삭제 금지) — `deleteFileFromWorkspace`
- [x] 중복 판정: 영상 고유 ID(aweme_id/modal_id) 우선 + 정규화 URL fallback (`extractVideoId`), 저장 파일명 ID 부착으로 덮어쓰기 방지
- [x] 스키마 v6 제목 이중화(originalTitle/translatedTitle), Desktop titleTranslate 적용
- [x] 브라우저 탭 아이콘: CozyBuilder 공통 C 아이콘(homepage 원본 복사, `src/app/icon.png`/`apple-icon.png`)
- [x] 검증: `npm run lint` / `npm run build` 통과 (수집 실동작은 확장·Douyin 로그인 환경에서 사장 확인)

## 다음 작업

- [ ] (승인 후) Vercel 프로젝트 연결·첫 배포, 도메인 `clipminer.cozybuilder.co.kr` 연결 (DNS 별도)
- [ ] 수집 신뢰성 보강: 백그라운드 탭 추출 안정화(로그인/지연 로드/페이지 구조), Xiaohongshu 수집
- [ ] 확장 배포 방식(Chrome Web Store 등록 여부) 검토
- [ ] 영상 편집(제목/URL/태그) UI 보강, 작업 폴더 파일 자동 매칭 고도화
- [ ] (선택) JSON Import/Export, Backup/Sync (이때 외부 DB/인증 재검토)
